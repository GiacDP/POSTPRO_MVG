...
!
 ...
 integer :: nx,ny,nz,nv,nv_stat_3d, j_block_start, j_block_end
!
 logical :: masterproc
!
...
CONTAINS 
  ...
  !-------------------------------------------------------------------------------------------------
  !
  SUBROUTINE write_vtk_general(ig_start, kg_start, nv_io, nx_io, ny_io, nz_io, w_aux_io, file_str, names, &
                  & nblocks_io, ncoords_io, mp_cart_io)

   implicit none

   integer, parameter :: int64_kind = selected_int_kind(2*range(1))

   INTEGER, INTENT(in) :: nv_io, nx_io, ny_io, nz_io, ig_start, kg_start, mp_cart_io
   INTEGER, DIMENSION(3), INTENT(in) :: nblocks_io, ncoords_io

   INTEGER :: nxmax_io, nymax_io, nzmax_io
   integer :: istore = 1
   integer :: l
   integer :: mpi_io_file
   integer :: filetype
   ...
   real(rkind), dimension(1:nx_io, 1:ny_io, 1:nz_io) :: w_tmp

   REAL(rkind), DIMENSION(1:nv_io, 1:nx_io, 1:ny_io, 1:nz_io), INTENT(in) :: w_aux_io
  ...
  !-------------------------------------------------------------------------------------------------
   ...
   nxmax_io = nx_io * nblocks_io(1)
   nymax_io = ny_io * nblocks_io(2)
   nzmax_io = nz_io * nblocks_io(3)

   size_real = storage_size(1._rkind)/8
   size_integer = storage_size(nxmax_io)/8
   ...
   gridsize_64 = int(size_real,int64_kind)*int(nxmax_io,int64_kind)*int(nymax_io,int64_kind)*&
                 int(nzmax_io,int64_kind)
   ...
   if (masterproc) then
    offset_x = 0
    offset_y = size_real*nxmax_io + storage_size(gridsize_64)/8
    offset_z = offset_y + size_real*nymax_io + storage_size(gridsize_64)/8
    ...
    xml_part = ' &
     ...
     &   //int2str(nxmax_io)//' +1 +'//int2str(nymax_io)//' +1 +'//int2str(nzmax_io)//'"> &
     &   <Piece Extent="+1 +'//int2str(nxmax_io)//&
     &    ' +1 +'//int2str(nymax_io)//' +1 +'//int2str(nzmax_io)//'"> &
     &    <Coordinates> &
    ...
    offset = offset_z + size_real*nzmax_io + storage_size(gridsize_64)/8
    do l=1,nv_io
    ...
    write(123) "_"
    write(123) size_real*int(nxmax_io,int64_kind) , xg(ig_start:ig_start+nxmax_io-1)
    write(123) size_real*int(nymax_io,int64_kind) , yg(1:nymax_io)
    write(123) size_real*int(nzmax_io,int64_kind) , zg(kg_start+1:nzmax_io+kg_start)
    flush(123)
    close(123)
   endif

   sizes(1) = nblocks_io(1)*nx_io
   sizes(2) = nblocks_io(2)*ny_io 
   sizes(3) = nblocks_io(3)*nz_io
   subsizes(1) = nx_io
   subsizes(2) = ny_io
   subsizes(3) = nz_io
   starts(1) = 0 + ncoords_io(1)*subsizes(1)
   starts(2) = 0 + ncoords_io(2)*subsizes(2) 
   starts(3) = 0 + ncoords_io(3)*subsizes(3) 
   ...
   do l=1,nv_io
    call MPI_BARRIER(mp_cart_io,iermpi)
    if (masterproc) then
    ...
    call MPI_BARRIER(mp_cart_io, iermpi)
    call MPI_FILE_OPEN(mp_cart_io,trim(file_prefix_)//chstore//'.vtr',MPI_MODE_RDWR,MPI_INFO_NULL,mpi_io_file,iermpi)
    ...
    w_tmp = w_aux_io(l, 1:nx_io, 1:ny_io, 1:nz_io)
  ...
  END SUBROUTINE write_vtk_general
